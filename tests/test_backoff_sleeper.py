...
def test_backoff_increases_when_elapsed_below_threshold(monkeypatch):
    ...
    monkeypatch.setattr(
        "time.monotonic",
        _set_monotonic([0.0, 0.0, 4.0, 4.0, 8.0, 8.0]),
    )
...
def test_backoff_resets_when_elapsed_meets_threshold(monkeypatch):
    sleeper = BackoffSleeper(
    ...
    monkeypatch.setattr(
        "time.monotonic",
        _set_monotonic([0.0, 0.0, 3.0, 3.0, 7.0, 7.0]),
    )
